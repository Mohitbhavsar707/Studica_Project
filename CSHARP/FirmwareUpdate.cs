...
namespace NetDFULib
{
	public class FirmwareUpdate : Win32Usb, IFirmwareUpdate
	{
		...
		// Detects STM32 DFU Device, and returns the bootloader id and flash size (in K) of the 
		// DFU Device.
		public bool IsDFUDeviceFound(out byte bootloader_id, out short flash_size_k)
		{
			//Console.Error.WriteLine("1here1");
			List<MappingSector> sectors;
			ushort nMaxBlkSize;
			IntPtr hDevice = OpenDFU_Device(out sectors, out nMaxBlkSize);     //XXX does this hDevice leak?
			//Console.Error.WriteLine("1here2");
			
			bootloader_id = 0;
			flash_size_k = 0;
			int block_fail = 0;
			bool got_bootloader_id = false;
			bool got_flash_size_k = false;
			

			if (hDevice != IntPtr.Zero) 
			{
				byte[] reg_value = new byte[2048];
				// A transfer size of 2048 (the maximum) is used
				// Bootloader ID (byte):  Address:  0x1FFF76DE.  Base:  0x1FFF0000, Block:  15, Offset:  0x6DE
				// (Documented in AppNote AN2606)
				// Flash Memory Size (short):  Address 0x1FF7A22.  Base:  0x1FFF0000, Block:  16, Offset:  0x222
				// Package Data (bits 8-10):  Address 0x1FFF7BF0.  Base:  0x1FFF0000, Block:  16, Offset:  0x3F0
				// (Documented in STM32F411/STM32F446 Reference Manual)
				uint system_memory_base_address = 0x1FFF0000;
				int num_blocks = 16;
				int bootloader_id_block = 14;
				int bootloader_id_offset = 0x6DE;
				int flash_memory_size_block = 15;
				int flash_memory_size_offset = 0x222;
				//int package_data_block = 15;
				//int package_data_offset = 0x3F0;
				int first_block = bootloader_id_block;
				SetAddressPointer(hDevice, system_memory_base_address);
				for (int BlockNumber = first_block; BlockNumber < num_blocks; BlockNumber++)
				{
					uint address = system_memory_base_address + ((uint)BlockNumber * 2048);
					if (STDFU_NOERROR == STDFU_Upload(ref hDevice, reg_value, (UInt32)reg_value.Length, (UInt16)(BlockNumber + 2)))
					{
						DFU_Status dfuStatus = new DFU_Status();
						STDFU_GetStatus(ref hDevice, ref dfuStatus);
						if (dfuStatus.bState != STATE_DFU_UPLOAD_IDLE) {
						...
						{
							if (BlockNumber == bootloader_id_block)
							{
								bootloader_id = reg_value[bootloader_id_offset];
								got_bootloader_id = true;
							} else if (BlockNumber == flash_memory_size_block)
							{
								short reg_val_low = reg_value[flash_memory_size_offset];
								short reg_val_high = reg_value[flash_memory_size_offset+1];
								reg_val_high <<= 8;
								flash_size_k = (short)(reg_val_high + reg_val_low);
								got_flash_size_k = true;
							}
						}
					}
				}
			}
			//Console.Error.WriteLine("1here3");
			return ((IntPtr.Zero != hDevice) && got_bootloader_id && got_flash_size_k);
		}
